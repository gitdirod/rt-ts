...
const NumberColorsBlock=({numberC})=> {
    ...
    const handleEdit = e => {
        ...
        const renameNumberColors = {
            _method: 'PUT',
            id: numberC.id,
            name: nameRef.current.value
        }
        if(!delWaiting){
            update('number_colors',renameNumberColors,setEditErrores,setEditState, setEditWaiting)
        }
    }
    useEffect(()=>{
        if(editState){
    ...
    const handleDel= e =>{
        ...
        if(!delWaiting){
            destroy('number_colors', delSize, setDelErrores, setDelState, setDelWaiting)
        }
    }
    useEffect(()=>{
        if(delState){
        ...
        >
            {
                ...
                >
                    <div className='flex gap-x-2 items-center rounded-md'>
                        ...
                        >
                            <img className="w-6 h-6 grey" src={clasificacion} alt="" />
                            Actualizar:
                        </label>
                        <input 
                            type="text" 
...
export default memo(NumberColorsBlock) 