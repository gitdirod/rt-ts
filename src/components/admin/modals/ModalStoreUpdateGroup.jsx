...
const ModalStoreUpdateGroup = forwardRef(({ group, onUpdated, onCancel }, ref) => {
    ...
    const handleSubmit = async e =>{
        ...
        ? await GroupService.update(group.id, gru) 
        : await GroupService.create(gru);
    ...
    <Box sx={style}>
        ...
        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
            <Box sx={{ display: 'flex', alignItems: 'flex-end', mb: 2 }}>
                <GridViewIcon sx={{ color: 'action.active', mr: 1 }} />
                <TextField
                label="Nombre"
                variant="standard"
                fullWidth
...
export default ModalStoreUpdateGroup;
